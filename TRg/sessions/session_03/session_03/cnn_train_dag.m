...
for p=1:numel(net.params)
  ...
  switch net.params(p).trainMethod
    ...
    case 'adagrad'
      if(isempty(net.params(p).accum_der))
          net.params(p).accum_der = 0.*net.params(p).der;
      end
      
      g2_p = net.params(p).der .* net.params(p).der;
      net.params(p).accum_der = net.params(p).accum_der + g2_p;
      kk = 1e2;
      step = (state.learningRate ./ (sqrt(1 + kk*net.params(p).accum_der) + eps));
      str0 = sprintf('min = %e | max = %e | avg = %e', min(step(:)), max(step(:)), mean(step(:)));
      %disp(str0);
      %pause;

      %avg = 1e1*norm(net.params(p).der(:));
      net.params(p).value = net.params(p).value - step .* net.params(p).der;
%step
