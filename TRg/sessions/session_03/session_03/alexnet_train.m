...
%  Demonstrated MatConNet on CIFAR-10 using DAG
	...
	%initNet(net, [1, 1e-2, 1e-2, 1e-2, 1e-2, 1e-2, 1e-2, 1e-2], 0*ones(1, 8));
	initNet_xavier(net)
	%initNet_He(net, 1);
...
function initNet_He(net, f)
	...
	for l=1:length(net.layers)
		...
		if(strcmp(class(net.layers(l).block), 'dagnn.Conv'))
			...
			[h,w,in,out] = size(net.params(f_ind).value);
			he_gain = sqrt(2/(size(net.params(f_ind).value,1)*size(net.params(f_ind).value,2)*size(net.params(f_ind).value,3))); % sqrt(1/fan_in)
